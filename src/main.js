...
function readActiveSessionStats() {
  try {
    ...
    // Pin to the active session's own transcript. Several `claude` processes
    // can write into this same project dir at once (the foreground chat plus
    // background/sub agents Husk launches), so picking "newest .jsonl by mtime"
    // on every 8s poll makes the Context Window readout flip between unrelated
    // sessions and blank out when a fresh agent file (no usage yet) wins the
    // race. Once we've locked the file this session is writing, keep reading it
    // so the percentage is stable and grows with the conversation; only
    // re-resolve when that file disappears.
