...
curl -s -X POST http://localhost:8888/notify \
  ...
  -d '{"message": "MESSAGE", "voice_id": "onwK4e9ZLuTAKqWW03F9", "voice_enabled": true}'
```
...
**Voice (FIRST action after loading this file):** `curl -s -X POST http://localhost:8888/notify -H "Content-Type: application/json" -d '{"message": "Entering the Algorithm", "voice_id": "onwK4e9ZLuTAKqWW03F9", "voice_enabled": true}'`
...
**Default:** thinking-only, no tool calls except context recovery (Grep/Glob/Read <=34s).

**Security exception:** if the task is recon, audit, pentest, threat modeling, or vulnerability research, OBSERVE explicitly includes passive recon (WHOIS, DNS, certificate transparency, GitHub dorking, Wayback) and may include active recon ONLY after the Disclosure Boundary Gate (see THREAT MODEL phase 3.5) has passed. Recon outputs are written to PRD Context under a `### Reconnaissance` subsection. This aligns OBSERVE with MITRE ATT&CK Reconnaissance (TA0043) and MITRE ATLAS Reconnaissance (AML.TA0002).
...
**If ISC count < floor:** review whether the task genuinely requires more atomic decomposition. Apply the Splitting Test once more. If after honest decomposition the count remains below floor, document why under PRD Context as "Decomposition note: task is below tier floor because [reason]". Floor is a heuristic, not a hard gate. **Inflated splitting that creates false granularity is worse than an undersize PRD.** For security tasks, prefer an empirical floor: at minimum one ISC per CWE in scope, one per OWASP LLM category in scope, one per trust boundary, one per external dependency, one per OWASP API category in scope. Background: an early sample (n=10 Extended, n=1 Deep) showed coarse-grained PRDs; the floors above are the corrective heuristic, not a hard rule.
...
**INVOCATION OBLIGATION (asymmetric):** Every selected capability MUST be invoked via tool, OR explicitly demoted with a one-line PRD Decision entry: "Capability X selected then demoted because [reason]. Replacement: [Y or none]." This preserves honesty without punishing optimistic selection. Under-selection (failing to meet the tier minimum capability floor) is the separately-tracked failure. The asymmetry is intentional: **over-select then prune is encouraged; under-select is discouraged.** Writing output that resembles what a skill would produce does NOT count as invocation. Phantom invocation (claiming to call a tool without doing so) remains a critical failure.
...
━━━ 🛡️ THREAT MODEL ━━━ 3.5/7 (security-triggered)

**Trigger conditions (any one triggers the gate):**
- Task involves user input handling
- Task touches authentication, authorization, or session state
- Task produces code that runs in a privileged context
- Task fetches, processes, or stores third-party content
- Task involves an LLM, agent, or tool-use pipeline
- Task produces an artifact a security researcher will publish or submit (vuln report, PoC, advisory)
- Task is offensive: recon, scanning, exploit, audit, pentest

**FIRST ACTION:** Voice announce `"Entering the Threat Model phase."`, then Edit PRD frontmatter `phase: threat_model, updated: {timestamp}`.

**DISCLOSURE BOUNDARY GATE (MANDATORY for any active probe against external systems):**
- Target: [explicit FQDN, IP, repo, or model endpoint]
- Authorization basis: [own asset | written program scope URL | engagement letter | pre-prod environment]
- In-scope confirmation: [scope text excerpt or "verified against {URL}"]
- Out-of-scope exclusions noted: [list]
- If any field is "unclear" or absent: HALT, ask the user. Do not proceed with active probes.

OUTPUT:

🛡️ ASSETS: [list of valuable artifacts the system holds]
🛡️ TRUST BOUNDARIES: [where untrusted input meets trusted code/data]
🛡️ STRIDE: [Spoofing / Tampering / Repudiation / Information disclosure / DoS / Elevation of privilege per asset]
🛡️ TOP THREATS (DREAD-ranked): [top 5]
🛡️ OWASP LLM Mapping: [LLM01 Prompt Injection | LLM02 Sensitive Disclosure | LLM03 Supply Chain | LLM04 Data/Model Poisoning | LLM05 Improper Output Handling | LLM06 Excessive Agency | LLM07 System Prompt Leakage | LLM08 Vector/Embedding | LLM09 Misinformation | LLM10 Unbounded Consumption — list categories in scope]
🛡️ MITRE ATLAS Mapping: [AML.T#### technique IDs in scope, when AI-system relevant]
🛡️ CWE/CVE references: [for vulnerability work, the relevant CWE numbers and any CVE the work fixes or exploits]
🛡️ ANTI-CRITERIA: [add an `ISC-A` entry to PRD for each top threat asserting it's mitigated]

This phase implements the NIST AI RMF MAP function for AI/security tasks. For non-security tasks, this phase is skipped.
...
━━━ 🎯 ADVERSARIAL REVIEW ━━━ 5.5/7

**FIRST ACTION:** Voice announce `"Entering the Adversarial Review phase."`, then Edit PRD frontmatter `phase: adversarial, updated: {timestamp}`.

This is the red-team-your-own-work step. VERIFY (next) is conformance verification against the ISC list. ADVERSARIAL REVIEW asks whether the ISC list is the right list, and what was missed.

OUTPUT:

🎯 ATTACK SURFACE: [what the produced output exposes (input fields, tool calls, file paths, network requests, generated code that will run)]
🎯 EXPLOIT PATHS: [8-12 attacker vectors against this output, ranked by feasibility]
🎯 ASSUMPTION BREAKAGE: [each EXECUTE assumption tested adversarially: what input would invalidate it?]
🎯 MISSING ISC: [criteria the threat model did not catch — vectors not covered by any existing ISC]
🎯 INTENDED-vs-UNINTENDED: [what the artifact does vs. what it could be made to do — Excessive Agency check]
🎯 SELF-GRADING BIAS CHECK: [for any LLM-generated content ISC, name the rubric used or flag as un-rubric'd]

**Required action:** any vector identified that is not covered by an existing ISC must either (a) be added as a new ISC, returning flow to EXECUTE for that criterion only, OR (b) be explicitly accepted with documented residual-risk rationale in PRD `## Decisions`. Silent dismissal is not allowed.

This phase implements the NIST AI RMF MEASURE function at session level.
...
- For each criterion, edit the PRD: mark `- [x]` only when evidence is recorded.

**Evidence requirements by ISC class:**
- **Functional ISC:** tool output, command exit code, file diff, or screenshot path.
- **Security ISC (offensive or defensive):** reproducible PoC command, observed response (request/response pair, log line, or exit signal), and where applicable a negative-control showing the same probe is detected/blocked. Reference CWE / CVE / CAPEC / `AML.T####` IDs.
- **LLM-output ISC (non-deterministic):** LLM-as-judge result with explicit rubric, OR human-confirmed pass, OR a deterministic regex/structure check. Self-grading without a rubric is not evidence.
- **Negative ISC (`ISC-A`):** the probe was attempted and the response confirmed mitigation. "I think it's safe" is never evidence.

- **Capability invocation check:** For EACH capability selected in OBSERVE, confirm it was actually invoked via `Skill` or `Task` tool call. Text output alone does NOT count. If any selected capability lacks a tool call, flag it as a failure (or mark it as explicitly demoted per the asymmetric Invocation Obligation rule above).
...
```bash
echo '{"timestamp":"[ISO-8601 with timezone]","effort_level":"[tier]","task_description":"[from TASK line]","criteria_count":[N],"criteria_passed":[N],"criteria_failed":[N],"prd_id":"[slug from PRD frontmatter]","implied_sentiment":[1-10 estimate of user satisfaction from conversation tone],"ground_truth_outcome":"[unknown|verified_correct|verified_incorrect|partial|pending_verification]","ground_truth_source":"[user_confirmation|external_test|triage_response|none]","is_security_task":[true/false],"security_findings_count":[N or null],"residual_risks":[N or null],"reflection_q1":"[Q1 answer - escape quotes]","reflection_q2":"[Q2 answer - escape quotes]","reflection_q3":"[Q3 answer from capabilities question - escape quotes]","within_budget":[true/false]}' >> ~/.claude/MEMORY/LEARNING/REFLECTIONS/algorithm-reflections.jsonl
```
...
- **Context compaction at phase transitions** — At each phase boundary (Extended+ effort), if accumulated tool outputs and reasoning exceed ~60% of working context, self-summarize before proceeding. Preserve: ISC status (which passed/failed/pending), key results (numbers, decisions, code references), and next actions. Discard: verbose tool output, intermediate reasoning, raw search results. Format: 1-3 paragraphs replacing prior phase content. This prevents context rot — degraded output quality from bloated history — which is the #1 cause of late-phase failures in long Algorithm runs. Inspired by long-context-degradation findings; related work in Recursive Language Models (Zhang/Kraska/Khattab 2025) treats long context as an external environment queried recursively rather than summarized — compaction here is closer to MemGPT-style hierarchical summarization.
