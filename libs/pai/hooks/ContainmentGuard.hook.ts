 ...
 *
 * Anything outside those zones must stay clean of the deployment's private
 * identity and infrastructure strings, listed in IDENTITY_PATTERNS below.
 *
 * TRIGGER: PreToolUse (matcher: Edit, Write, MultiEdit)
 * EXIT CODES: 0 = allow, 2 = deny (blocks the write)
 *
 * Test safely:
 *   echo '{"tool_name":"Write","tool_input":{"file_path":"/path/to/.claude/hooks/demo.ts","content":"const u = \"private-string\";"}}' | bun run hooks/ContainmentGuard.hook.ts; echo $?
 */
...
// Private identity and infrastructure strings that must never leak outside the
// containment zones. Empty in the bundled framework; populate per deployment.
