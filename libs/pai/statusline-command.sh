...
calc_bar_width() {
    ...
    case "$mode" in
        ...
        mini)
            ...
            ;;
        normal)
            prefix_len=13   # "◐  CONTEXT   " (icon + 2sp + 8-char label + 2sp)
            suffix_len=28   # "  XX%  XXXK / XXXXK · fresh" (account for token+state suffix)
...
case "$MODE" in
    ...
    mini)
        ...
        ;;
    normal)
        # Fixed-grid normal mode: ICON  LABEL_8w  DATA  (prefix = 13 chars)
        # Header: truncate session_display to 25 chars, equal-length dashes
        local_brand="PAI ${PAI_VERSION}"
        sess_trunc="${session_display}"
        if [ -n "$sess_trunc" ] && [ ${#sess_trunc} -gt 25 ]; then
            sess_trunc="${sess_trunc:0:24}…"
        fi
        if [ -n "$sess_trunc" ]; then
            local_left="── ATLAS · ${sess_trunc^^}"
        else
            local_left="── ATLAS"
        fi
        local_right="${local_brand} ──"
        local_fill=$((72 - ${#local_left} - ${#local_right} - 2))
        [ "$local_fill" -lt 2 ] && local_fill=2
        local_dashes=$(printf '%*s' "$local_fill" '' | sed 's/ /─/g')
        printf "${SLATE_600}── ${PAI_A}ATLAS${RESET}"
        if [ -n "$sess_trunc" ]; then
            printf " ${SLATE_600}·${RESET} ${PAI_SESSION}${sess_trunc^^}${RESET}"
        fi
        printf " ${SLATE_600}${local_dashes} ${PAI_A}${local_brand}${RESET} ${SLATE_600}──${RESET}\n"

        # WHERE: location · time · weather (label pad 8)
        if [ "$location_city" = "$location_state" ] || [ -z "$location_state" ]; then
            location_full="$location_city"
        else
            location_full="${location_city}, ${location_state}"
        fi
        printf "${PAI_LABEL}◷${RESET}  ${PAI_LABEL}%-8s${RESET}  ${PAI_CITY}%s${RESET}  ${SLATE_600}·${RESET}  ${PAI_TIME}%s${RESET}  ${SLATE_600}·${RESET}  ${PAI_WEATHER}%s${RESET}\n" "WHERE" "$location_full" "$current_time" "$weather_str"

        # BUILD: software versions on first line
        printf "${SLATE_400}▣${RESET}  ${SLATE_400}%-8s${RESET}  ${SLATE_400}Claude${RESET} ${PAI_A}%s${RESET}  ${SLATE_600}·${RESET}  ${SLATE_400}PAI${RESET} ${PAI_A}%s${RESET}  ${SLATE_600}·${RESET}  ${SLATE_400}ALG${RESET} ${PAI_A}%s${RESET}\n" "BUILD" "$cc_version" "$PAI_VERSION" "$ALGO_VERSION"

        # TOOLS: skills / workflows / hooks counts, full words for clarity
        printf "${WIELD_PRIMARY}⌬${RESET}  ${WIELD_PRIMARY}%-8s${RESET}  ${SLATE_300}%s${RESET} ${WIELD_ACCENT}Skills${RESET}  ${SLATE_600}·${RESET}  ${SLATE_300}%s${RESET} ${WIELD_WORKFLOWS}Workflows${RESET}  ${SLATE_600}·${RESET}  ${SLATE_300}%s${RESET} ${WIELD_HOOKS}Hooks${RESET}\n" "TOOLS" "$skills_count" "$workflows_count" "$hooks_count"
        ;;
esac
...
case "$MODE" in
    ...
    normal)
        bar=$(render_context_bar $bar_width $display_pct)
        # Token count and fresh-state suffix
        tokens_in_k=$(( (${total_input:-0} + 500) / 1000 ))
        max_in_k=$(( (${context_max:-200000} + 500) / 1000 ))
        if [ "$raw_pct" -lt 5 ]; then
            state_suffix=" ${SLATE_600}·${RESET} ${EMERALD}fresh${RESET}"
        elif [ "$raw_pct" -ge 80 ]; then
            state_suffix=" ${SLATE_600}·${RESET} ${ROSE}near limit${RESET}"
        else
            state_suffix=""
        fi
        # Fixed-grid CONTEXT row
        printf "${CTX_PRIMARY}◐${RESET}  ${CTX_SECONDARY}%-8s${RESET}  ${bar}  ${pct_color}%s%%${RESET}  ${SLATE_500}%sK / %sK${RESET}${state_suffix}\n" "CONTEXT" "$raw_pct" "$tokens_in_k" "$max_in_k"
        ;;
esac
printf "${SLATE_600}────────────────────────────────────────────────────────────────────────${RESET}\n"
...
" 2>/dev/null)"
    ...
    # Extra usage display (Max plan overage credits, cents)
    extra_display=""
    extra_display_normal=""
    if [ "$usage_extra_enabled" = "true" ]; then
        ...
        extra_display_normal="EXTRA \$${extra_used_int}/${extra_limit_fmt}"
    fi
    ...
    ws_display_normal="API \$${ws_cost_dollars}"
    ...
    case "$MODE" in
        ...
        mini)
            ...
            ;;
        normal)
            # Fixed-grid USAGE — 3 rows, descriptive labels, "Refresh at" instead of cryptic →
            # Row 1: 5h Usage XX%  ·  Refresh at HH:MM
            printf "${USAGE_PRIMARY}▤${RESET}  ${USAGE_LABEL}%-8s${RESET}  " "USAGE"
            printf "${USAGE_RESET}5h Usage${RESET} ${usage_5h_color}%s%%${RESET}" "$usage_5h_int"
            if [ -n "$reset_5h_time" ] && [ "$reset_5h_time" != "—" ]; then
                printf "  ${SLATE_600}·${RESET}  ${USAGE_RESET}Refresh at${RESET} ${SLATE_500}${reset_5h_time}${RESET}"
            fi
            printf "\n"
            # Row 2: Weekly Usage XX%  ·  Refresh at DDD HH:MM (13-space indent for data column)
            printf "             "
            printf "${USAGE_RESET}Weekly Usage${RESET} ${usage_7d_color}${usage_7d_int}%%${RESET}"
            if [ -n "$reset_7d_time" ] && [ "$reset_7d_time" != "—" ]; then
                printf "  ${SLATE_600}·${RESET}  ${USAGE_RESET}Refresh at${RESET} ${SLATE_500}${reset_7d_time}${RESET}"
            fi
            printf "\n"
            # Row 3: financials (only if any are present)
            cont_parts=()
            [ -n "$ws_display_normal" ] && cont_parts+=("${USAGE_EXTRA}${ws_display_normal}${RESET}")
            [ -n "$extra_display_normal" ] && cont_parts+=("${USAGE_EXTRA}${extra_display_normal}${RESET}")
            [ -n "$session_cost_str" ] && cont_parts+=("${USAGE_EXTRA}Session ${session_cost_str}${RESET}")
            if [ ${#cont_parts[@]} -gt 0 ]; then
                printf "             "  # 13-space indent
                cont_first=1
                for p in "${cont_parts[@]}"; do
                    [ "$cont_first" -eq 1 ] && cont_first=0 || printf "  ${SLATE_600}·${RESET}  "
                    printf "%b" "$p"
                done
                printf "\n"
            fi
...
case "$MODE" in
    ...
    normal)
        # Fixed-grid PWD row
        printf "${GIT_PRIMARY}◇${RESET}  ${GIT_PRIMARY}%-8s${RESET}  ${GIT_DIR}%s${RESET}" "PWD" "$dir_name"
        if [ "$is_git_repo" = true ]; then
            printf "  ${SLATE_600}·${RESET}  ${SLATE_500}⎇${RESET} ${GIT_VALUE}${branch}${RESET}"
            [ -n "$age_display" ] && printf "  ${SLATE_600}·${RESET}  ${SLATE_500}⌛${RESET} ${age_color}${age_display}${RESET}"
            [ "$stash_count" -gt 0 ] && printf "  ${SLATE_600}·${RESET}  ${SLATE_500}⊘${RESET} ${GIT_STASH}${stash_count}${RESET}"
            if [ "$ahead" -gt 0 ] || [ "$behind" -gt 0 ]; then
                printf "  ${SLATE_600}·${RESET}  ${SLATE_500}⇅${RESET} "
...
case "$MODE" in
    ...
    mini)
        ...
        ;;
    normal)
        # Fixed-grid MEMORY row. Each cell labeled: glyph + count + descriptive word.
        printf "${LEARN_PRIMARY}◎${RESET}  ${LEARN_SECONDARY}%-8s${RESET}  " "MEMORY"
        printf "${LEARN_SESSIONS}⊕${RESET} ${SLATE_300}${sessions_count}${RESET} ${LEARN_SESSIONS}sessions${RESET}"
        printf " ${SLATE_600}·${RESET} ${LEARN_SIGNALS}✦${RESET} ${SLATE_300}${ratings_count}${RESET} ${LEARN_SIGNALS}ratings${RESET}"
        printf " ${SLATE_600}·${RESET} ${LEARN_WORK}▤${RESET} ${SLATE_300}${work_count}${RESET} ${LEARN_WORK}work${RESET}"
...
if [ -f "$RATINGS_FILE" ] && [ -s "$RATINGS_FILE" ]; then
    ...
    if [ "$cache_valid" = true ]; then
      ...
      def make_sparkline($period_start):
        . as $all | ($now - $period_start) as $dur | ($dur / 30) as $sz |
        [range(30) | . as $i | ($period_start + ($i * $sz)) as $s | ($s + $sz) as $e |
          [$all[] | select(.epoch >= $s and .epoch < $e) | .rating] |
          if length == 0 then "\u001b[38;2;71;85;105m·\u001b[0m" else (add / length) | to_bar end
        ] | join("");
    ...
    if [ "$total_count" -gt 0 ] 2>/dev/null; then
        ...
        case "$MODE" in
            ...
            mini)
                ...
                ;;
            normal)
                # Fixed-grid LEARNING row
                src_human=$([ "$latest_source" = "explicit" ] && echo "manual" || echo "auto")
                printf "${LEARN_LABEL}✿${RESET}  ${LEARN_LABEL}%-8s${RESET}  " "LEARNING"
                printf "${SLATE_500}now${RESET} ${LATEST_COLOR}${latest}${RESET} ${SLATE_500}(${src_human})${RESET} ${trend_color}${trend_icon}${RESET}"
                # Suppress empty buckets entirely — no "15m: —" noise
                [ "$q15_avg" != "—" ]   && printf "  ${SLATE_600}·${RESET}  ${SIGNAL_PERIOD}15m${RESET} ${Q15_COLOR}${q15_avg}${RESET}"
                [ "$hour_avg" != "—" ]  && printf "  ${SLATE_600}·${RESET}  ${SIGNAL_PERIOD}1h${RESET} ${HOUR_COLOR}${hour_avg}${RESET}"
                [ "$today_avg" != "—" ] && printf "  ${SLATE_600}·${RESET}  ${SIGNAL_PERIOD}1d${RESET} ${TODAY_COLOR}${today_avg}${RESET}"
                [ "$week_avg" != "—" ]  && printf "  ${SLATE_600}·${RESET}  ${SIGNAL_PERIOD}1w${RESET} ${WEEK_COLOR}${week_avg}${RESET}"
                [ "$month_avg" != "—" ] && printf "  ${SLATE_600}·${RESET}  ${SIGNAL_PERIOD}1mo${RESET} ${MONTH_COLOR}${month_avg}${RESET}"
                printf "\n"

                # Sparklines: render only rows with data; indent 13 spaces to align with data column
                [ "$q15_avg" != "—" ]   && printf "             ${SLATE_600}·${RESET} ${SIGNAL_PERIOD}%-4s${RESET} %s\n" "15m"  "$q15_sparkline"
                [ "$hour_avg" != "—" ]  && printf "             ${SLATE_600}·${RESET} ${SIGNAL_PERIOD}%-4s${RESET} %s\n" "1h"   "$hour_sparkline"
                [ "$today_avg" != "—" ] && printf "             ${SLATE_600}·${RESET} ${SIGNAL_PERIOD}%-4s${RESET} %s\n" "1d"   "$day_sparkline"
                [ "$week_avg" != "—" ]  && printf "             ${SLATE_600}·${RESET} ${SIGNAL_PERIOD}%-4s${RESET} %s\n" "1w"   "$week_sparkline"
                [ "$month_avg" != "—" ] && printf "             ${SLATE_600}·${RESET} ${SIGNAL_PERIOD}%-4s${RESET} %s\n" "1mo"  "$month_sparkline"
                ;;
        esac
    else
