...
test('startup mirrors an existing claude agent file to copilot, verbatim', async () => {
  const homeDir = makeHome({
    profiles: [{ id: 'p1', name: 'Bappsec', description: 'sec agent', systemPrompt: 'body', builtin: false }],
  });
  // A rich, hand-authored claude agent file (extra frontmatter) already exists.
  const rich = '---\nname: Bappsec\ndescription: sec agent\ncolor: red\n---\n\nYou are a senior security engineer.\n';
  fs.writeFileSync(path.join(homeDir, '.claude', 'agents', 'bappsec.md'), rich);
  ...
  const copilotFile = path.join(homeDir, '.copilot', 'agents', 'bappsec.md');
  expect(fs.existsSync(copilotFile)).toBe(true);
