#!/usr/bin/env bash
# Husk uninstaller. Removes desktop integration, config, voice data, and caches.
# The project directory and node_modules are left intact (re-install via ./install.sh
# uses them). Pass --keep-data to preserve config and voice data.
set -e
...
# Always-remove: desktop integration
remove_path "$WRAPPER"
remove_path "$DESKTOP_FILE"
remove_path "$ICON_FILE"
remove_path "$TMP_DIR"

# User data: removed by default, kept only if --keep-data passed
...
if command -v gsettings >/dev/null 2>&1; then
    ...
    if echo "$CURRENT" | grep -q "$APP_ID.desktop"; then
        ...
        gsettings set org.gnome.shell favorite-apps "$NEW" 2>/dev/null && echo -e "${C_OK}removed${C_RST} from favorites" || true
    fi
fi

# Refresh system caches
command -v update-desktop-database >/dev/null 2>&1 && update-desktop-database "$HOME/.local/share/applications" 2>/dev/null || true
command -v gtk-update-icon-cache >/dev/null 2>&1 && gtk-update-icon-cache -tf "$HOME/.local/share/icons/hicolor" 2>/dev/null || true
